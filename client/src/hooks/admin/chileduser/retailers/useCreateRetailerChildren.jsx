...
import { createretailersChild } from "../../../../stores/actions/retailerAction";
import { Country, State, City } from "country-state-city";
const useCreateRetailerChildren = (adminParent) => {
    ...
    const onSubmit = async (childAdminData) => {
        ...
        try {
            await dispatch(createretailersChild({ childAdminData, parentAdminId })).unwrap();
            setShowCreateMessage("User created successfully!");

            setTimeout(() => navigate(-1), 1000);
            reset();
        } catch (err) {
            const superdistributorsCreateError = err?.response?.data?.message || err?.message || "Failed to create player.";
...
export default useCreateRetailerChildren;
