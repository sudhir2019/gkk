...
import { createdistributors } from "../../../../stores/actions/distributorAction";
import useFetchAllSuperDistributor from "../superdistributors/useFetchAllSuperDistributor";
import { Country, State, City } from "country-state-city";
const useCreateSuperDistributors = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await dispatch(createdistributors({ ...data })).unwrap();
            setShowCreateMessage("User created successfully!");

            setTimeout(() => navigate(-1), 1000);
            reset();
        } catch (err) {
            const superdistributorsCreateError = err?.response?.data?.message || err?.message || "Failed to create area manager.";
...
export default useCreateSuperDistributors;
