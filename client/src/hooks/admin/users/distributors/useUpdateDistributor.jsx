...
import useFetchDistributorsById from "./useFetchDistributorById";
import { updatedistributors } from '../../../../stores/actions/distributorAction';
import useFetchAllSuperDistributor from '../superdistributors/useFetchAllSuperDistributor';

const useUpdateDistributors = (distributorsId) => {
    const dispatch = useDispatch();
    const { distributor } = useFetchDistributorsById(distributorsId);  // Fetch Distributors data
    ...
    // Prefill form when Distributors data is available
    useEffect(() => {
        if (distributor) {
    ...
    const onSubmit = async () => {
        ...
        try {
            await dispatch(updatedistributors({ distributorsId, updatedFields })).unwrap();
            // console.log("✅ API Success:", updatedFields);
            setShowUpdateMessage("User updated successfully!");
            setTimeout(() => window.history.back(), 1000);
        } catch (error) {
            // console.error("❌ API Error:", error);
            const DistributorsUpdateerror = distributorsError || error.message || "server error";
            setShowUpdateError(DistributorsUpdateerror);
        } finally {
...
export default useUpdateDistributors;
