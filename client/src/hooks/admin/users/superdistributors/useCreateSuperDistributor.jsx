...
import { createsuperdistributors } from "../../../../stores/actions/superDistributorAction";
...
const useCreateSuperDistributors = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            await dispatch(createsuperdistributors({ ...data })).unwrap();
            setShowCreateMessage("User created successfully!");

            setTimeout(() => navigate(-1), 1000);
            reset();
        } catch (err) {
            const superdistributorsCreateError = err?.response?.data?.message || err?.message || "Failed to create super area manager.";
