...
import { updateusers } from '../../../../stores/actions/otcActions';
import useFetchAllAdmins from "../admin/useFetchAllAdmins";

const useUpdateOtc = (usersId) => {
    const dispatch = useDispatch();
    const { otc } = useFetchUsersById(usersId);  // Fetch users data
    ...
    useEffect(() => {
        if (otc) {
            ...
            reset({
                firstName: otc.firstName || "",
                lastName: otc.lastName || "",
                email: otc.email || "",
                phone: otc.phone || "",
                // Ensure date is properly formatted
                dateOfBirth: formatDateForInput(otc.dateOfBirth),
                country: otc.country || "",
                ...
                occupation: otc.occupation || "",
                note: otc.note ?? "add notes",
                userStatus: otc.userStatus === true ? "true" : "false",
            });
            setSelectedCountry(otc.country || "");
            setSelectedState(otc.state || "");
            setSelectedCity(otc.city || "");
        }
    }, [otc, reset]);
    ...
    const getUpdatedValues = () => {
        ...
        Object.keys(formValues).forEach((key) => {
            ...
            if (key === 'dateOfBirth') {
                const adminDate = otc.dateOfBirth
                    ? new Date(otc.dateOfBirth).toISOString().split('T')[0]
                    : '';
            ...
            if (key === "password") {
                if (formValue && formValue !== otc[key]) {
                    updatedValues[key] = formValue;
                }
                return;
            }
            ...
            if (formValue !== undefined && formValue !== otc[key]) {
                updatedValues[key] = formValue;
            }
        });
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            const usersUpdateerror = otcError || error.message || "server error";
