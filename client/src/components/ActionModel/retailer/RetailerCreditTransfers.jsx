...
import useRetailersCreditTransfer from "../../../hooks/admin/users/retailers/useRetailersCreditTransfer"; // Adjust path as necessary
import { ScaleLoader } from "react-spinners"
export default function RetailerCreditTransfers() {
    const { any } = useParams();
    const {
        register,
        handleSubmit,
        ...
        errors,
        credit
    } = useRetailersCreditTransfer(any);
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className="form-group d-flex">
                                <div className="col-sm-4 offset-lg-3">
                                    <h4 className="breadcrumb bg-slate-700 text-white">User: {retailer?.username || ""}</h4>
                                    <br />
                                    <h4 className="breadcrumb  bg-slate-700 text-white">
                                        Individual Credit: {(retailer?.walletBalance || 0).toFixed(2)}
                                    </h4>
                                    <br />
                                    <h4 className="breadcrumb  bg-slate-700 text-white">
