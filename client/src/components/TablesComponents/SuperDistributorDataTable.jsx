...
const SuperDistributorDataTable = ({ superdistributers, openModal, handleActivateDeactivate, handleDelete }) => {
        ...
        },
        {
            ...
            selector: row => (
                <div className='w-100 d-flex flex-row justify-content-between align-items-center'>
                    <Link to={`../superareamanager/players/details/${row?._id}`} className="text-decoration-none">{row?.username}</Link>
                    <span>&nbsp;</span>
    ...
    const handleExport = () => {
        const exportData = superdistributers.map(superDistributer => ({
            ...
            'Points': superDistributer?.wallet.map(point => point.individualCredit).join(", "),
        ...
        XLSX.writeFile(wb, 'superareamanagerList.xlsx');
    };
...
export default SuperDistributorDataTable;
