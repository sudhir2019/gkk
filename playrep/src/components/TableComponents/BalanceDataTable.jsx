...
const BalanceDataTable = ({
    ...
    const loadModelData = async (id) => {
        try {
            const response = await GET(`/playrep/balancereport?id=${id}&agentId=`);
    ...
    const calculateTotals = () => {
        ...
        columnAttributes.forEach(attr => {
            ...
            if (['int', 'float'].includes(dataType)) {
                totals[columnName] = demoData.reduce((acc, row) => acc + (parseFloat(row[columnName]) || 0), 0);
            }
        });
        return totals;
    };
    ...
    const renderPagination = () => {
        ...
        return (
            <table style={{ width: '100%' }}>
                <BalanceModel modelData={modelData} isOpen={showModal} onClose={() => setShowModal(false)} />
                <tbody>
                    <tr>
