...
const TransferredNotReceivedDataTable = ({
    demoData = [],
    keyColumn = "",
    gridStyle = {
        headerClass: "",
    ...
    const columnHeaders = [
        "Sr No", "ID", "TransferTo", "Type", "Amount", "Transfer Time"
    ];

    const columnAttributes = [
        { columnName: "SrNo", dataType: "int" },
        { columnName: "id", dataType: "string" },
        { columnName: "TransferTo", dataType: "string" },
    ...
    useEffect(() => {
        const transformedData = demoData.map((object, index) => ({
            SrNo: index + 1,
            id: object.toUserId?.username?.toString() || "N/A",
            TransferTo: `${object.toUserId?.firstName} ${object.toUserId?.lastName}`,
            Type: 'Credit',
...
export default TransferredNotReceivedDataTable;
