...
const ManagerMailDataTable = ({
    ...
    const loadModelData = async (id) => {
        try {
            const response = await GET(`/playrep/mailreport?id=${id}&fromDate=${fromDate}&toDate=${toDate}`);
    ...
    const renderPagination = () => {
        ...
        return (
            <table style={{ width: '100%' }}>
                <ManagerModel modelData={modelData} isOpen={showModal} onClose={() => setShowModal(false)} />
                <tbody>
                    <tr>
...
export default ManagerMailDataTable;
