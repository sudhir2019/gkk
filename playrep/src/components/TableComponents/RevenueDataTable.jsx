...
const RevenueDataTable = ({
    ...
    useEffect(() => {
        ...
        calculatePagination(demoData.length);
    }, [demoData]);
        ...
        sortedData.sort((a, b) => {
            ...
            if (direction === 'asc') {
                switch (dataType) {
                    ...
                    case 'string': return aValue?.toString().localeCompare(bValue?.toString());
                ...
                switch (dataType) {
                    ...
                    case 'string': return bValue?.toString().localeCompare(aValue?.toString());
    ...
    const calculateTotals = () => {
        ...
        columnAttributes.forEach(attr => {
            ...
            if (['int', 'float'].includes(dataType)) {
                ...
                const sum = demoData.reduce((acc, row) => acc + (parseFloat(row[columnName]) || 0), 0);
                if (totals[columnName] !== "Revenue %") {
                    totals[columnName] = parseFloat(sum.toFixed(2));
                }
    ...
    const loadModelData = async (id,fromDatee,toDatee) => {
        try {
            const response = await GET(`/playrep/revenuereportbyid?id=${authUser._id}&fromDate=${fromDatee}&toDate=${toDatee}`);
    ...
    const handleModel = (e, row) => {
        console.log(row)
        e.preventDefault();
        loadModelData(row.id,row.fromDate,row.toDate);
        setShowModal(true);
    ...
    const renderPagination = () => {
        ...
        return (
            <table style={{ width: '100%' }}>
                <RevenueModel modelData={modelData} isOpen={showModal} onClose={() => setShowModal(false)} />
                <tbody>
                    <tr>
                                    ...
                                    <td align="center" key={colIndex}>
                                        {editableRow === rowIndex && isEditing ? (
                                            ...
                                            col.dataType === 'custom' ? (
                                                ...
                                                <span style={{ color: '#666666' }}>{row[col.columnName]}</span>
